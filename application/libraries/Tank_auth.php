...
class Tank_auth {
    ...
    function login($login, $password, $remember, $login_by_username, $login_by_email) {
        if ((strlen($login) > 0) AND (strlen($password) > 0)) {
           ...
           // fail - wrong password
                ...
                //hasher password check . Uncomment this line and line 66 to enable check hasher . Update utk login mykj user.
                } 
                elseif($this->is_max_login_attempts_exceeded($login)) {
                    $this->error = array('login' => 'auth_ip_block');
                   }
    ...
    function is_adminperalatan($activated = TRUE) {
        $user_level_id = $this->ci->users->get_user_level_by_name("adminperalatan");
