...
class Booking_model extends CI_Model {
    ...
    var $table_select_room_name = 'rooms.name as room_name, rooms.location, rooms.building, rooms.floor, rooms.capacity';
    ...
    var $table_transport_reference_select = 'booking_transports.booking_transport_id,booking_transports.secretariat,booking_transports.destination_from,booking_transports.destination_to,booking_transports.transport_purpose,booking_transports.total_passenger,booking_transports.trip_type,booking_transports.transports,';
...
//    }
function getUserFkId($id) {
        $this->db->select("{$this->table}.fk_user_id");
        $this->db->from($this->table);
        $this->db->where("{$this->table_fk_booking_id} = $id");
        $this->db->limit(1);
    ...
    function getRecordForRoomById($id) {
        ...
        {$this->table_reference_booking_equipments}.secretariat AS equipment_secretariat, rooms.fk_user_id AS room_owner ,rooms.name
       ")
                ->from($this->table)
                ->join($this->table_reference_booking_rooms, "{$this->table_fk_booking_id} = {$this->table_reference_booking_rooms_pk_id}")
    ...
    function getRecordsForFoodByDate($params) {
        ...
        if (isset($params['end_date'])) {
            $this->db->where("booking.start_date BETWEEN '" . pg_escape_string($params['start_date']) . " 00:00:00' AND '" . pg_escape_string($params['end_date']) . " 23:59:00'");
        } else {
            $this->db->where("DATE_FORMAT(booking.start_date, '%Y-%m-%d') = '" . pg_escape_string($params['start_date']) . "'");
        }
...
//echo $this->db->last_query();exit;
    ...
    function getRecordsForTransportByDate($params) {
        ...
        $this->db->select("user_profiles.full_name, {$this->table}.*,{$this->table_transport_reference_select}, user_profiles.contact_office , (select user_profiles.full_name from user_profiles where user_profiles.fk_user_id = booking.driver) as drivername")
                ->from($this->table)
        ...
        $this->db->where("{$this->table_field_status} = 1");
        $this->db->order_by('booking.driver ASC');
        $this->db->order_by('booking.start_date DESC');
    ...
    function getRecordsForRoomsByDate($params) {
        $where_clause = "";

        $this->db->select("{$this->table}.*,{$this->table_reference_booking_rooms}.*, user_profiles.full_name, user_profiles.contact_office,user_profiles.cawangan_name,user_profiles.department_name, {$this->table_reference_rooms}.*")
                ->from($this->table)
                ->join($this->table_reference_booking_rooms, "{$this->table_fk_booking_id} = {$this->table_reference_booking_rooms_pk_id}")
                ->join('user_profiles', "user_profiles.fk_user_id = booking.fk_user_id")
                ->join($this->table_reference_rooms, "{$this->table_reference_booking_rooms_pk_room_id} = {$this->table_reference_rooms_pk_id}")
                ->where("{$this->table_field_booking_type} = 1 $where_clause");
    ...
    function getAllRecordsForTransport($params, $check_user = true) {
        ...
        if ($check_user):
            if ($this->tank_auth->is_admin()):
                $where_clause = "";
            elseif ($this->tank_auth->is_subadmin()):
                $where_clause = " AND {$this->table_user_id} = {$this->tank_auth->get_user_id()}"; // Purposely make this fail
            elseif ($this->tank_auth->is_normal_user()):
                $where_clause = " AND {$this->table_user_id} = {$this->tank_auth->get_user_id()}";
            endif;
        endif;
