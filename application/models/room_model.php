...
class Room_model extends CI_Model {
    ...
    //Ake . Add get record for capacity
    function getRecordByCap($capid) {
        
               $this->db->select('capacity');
               $this->db->where('room_id',$capid);
		...
		$values[] = "'$val'";
	     ...
	     $values = implode(',', $values);
            ...
            if (empty($id)) {
                ...
                //remove this line kalau tuka blk ke sql . $query ni letak dkt luwa else , buang fields2 
                $query = "$query_type {$this->table} ($fields) VALUES ($values) ";
            } else {
                $query_type = 'UPDATE';     
                $query = "$query_type {$this->table} SET $fields2";
            }
