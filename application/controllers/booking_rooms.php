            ...
            if ($this->form_validation->run() != FALSE) {
               ...
               // ake utk filter total kapisiti bilik tidak melebihi total user input.
                ...
                if (!empty($food_type_list)):
                    ...
                    $db_data_booking_foods['food_purpose'] = mysql_real_escape_string($this->input->post('food_purpose'));
                ...
                if (!empty($equipment_type_list)):
                    ...
                    $db_data_booking_equipments['secretariat'] = $this->db->escape_str($this->input->post('secretariat'));
                ...
                $db_data_booking_rooms['room_purpose'] = $this->db->escape_str($this->input->post('room_purpose'));
                $db_data_booking_rooms['total_from_agensi'] = $this->input->post('total_from_agensi');
                $db_data_booking_rooms['total_from_nonagensi'] = $this->input->post('total_from_nonagensi');
...
//                $db_data_booking_rooms['fk_user_id'] = $db_data_booking['fk_user_id'];
                ...
                $book_email['end_date'] = $db_data_booking['end_date'];
                $book_email['display_name'] = $GetbookUsers->full_name;
                $book_email['room_name'] = $GetbookRoomName->name;
                $book_email['fk_book_id'] =$id;
                $book_email['site_name'] = $site_name;
                $book_email['chairman'] = $db_data_booking_rooms['chairman'];
                $book_email['secretariat'] = $db_data_booking_rooms['secretariat'];
                $book_email['description'] = $db_data_booking_rooms['description'];
        ...
        foreach ($user_list as $user):
            ...
            $user_list_tmp[] = '"' . htmlspecialchars_decode($user->full_name) . '"';
        endforeach;
    ...
    function view($id) {
        ...
        $data['getname'] = $this->User_model->getEmelNotelSec($booking->secretariat);
    ...
    function sendNotification2($email_booker, $booking_notification_type, $site_name, $messageNoti) {
        $this->email->from($this->config->item('webmaster_email', 'tank_auth'), $this->config->item('website_name', 'tank_auth'));
        $this->email->reply_to($this->config->item('webmaster_email', 'tank_auth'), $this->config->item('website_name', 'tank_auth'));
        $this->email->to($email_booker);
        $this->email->subject(sprintf($this->lang->line('auth_subject_' . $booking_notification_type), $site_name));
        $this->email->message($messageNoti);
