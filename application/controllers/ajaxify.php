    ...
    function check_availability($booking_type) {
        ...
        if ($booking_type == $this->booking_types['room']) {
            ...
            if ($this->input->is_ajax_request() && $this->input->post('start_date') && $this->input->post('end_date')) {
                $where_conditions =
                        "(" .
                        "(booking.start_date >= '" . date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $this->input->post('start_date')))) . "' AND booking.start_date < '" . date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $this->input->post('end_date')))) . "') OR " .
                        "(booking.start_date <= '" . date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $this->input->post('start_date')))) . "' AND booking.end_date > '" . date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $this->input->post('start_date')))) . "') OR " .
                        "(booking.end_date > '" . date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $this->input->post('start_date')))) . "' AND booking.end_date <= '" . date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $this->input->post('end_date')))) . "')" .
                        ") AND " .
                        "booking.booking_status >=0 AND booking.booking_status != 6 AND booking.booking_status != 8 AND " .
//                        "booking.full_day != 1 AND " .
                ...
                if (count($available_rooms) == 0) {
                    ...
                    foreach ($available_rooms as $available_room):
                        ...
                        //$kapasiti = 'Kapasiti :' $available_room['capacity'];
                        $option_string .= "<option value='{$available_room['room_id']}'>{$available_room['building']}, {$available_room['floor']}, {$available_room['name']}, {$available_room['capacity']}</option>\n";
        ...
        if ($booking_type == $this->booking_types['equipment']) {
            if ($this->input->is_ajax_request() && $this->input->post('start_date') && $this->input->post('end_date')) {
                $where_conditions =
                        ...
                        "(booking.end_date > '" . date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $this->input->post('start_date')))) . "' AND booking.end_date <= '" . date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $this->input->post('end_date')))) . "')" .
                        ") AND " .
                        "booking.booking_status >=0 AND booking.booking_status != 6 AND booking.booking_status != 8";
    ...
    public function getUserList() {
        ...
        $user_list = $this->User_model->getAllRecords("user_profiles.full_name != ''");
        $users = array();
