...
//        $config['total_rows'] = $this->User_model->getCountUsers($params1);
...
//        $data['pagination'] = $pagination;
    ...
    function download() {
        $data = file_get_contents(FCPATH . 'assets/template/Fail Templat Excel MyBooking.xls');
        $name = 'Fail Templat Excel MyBooking.xls';
...
//            redirect('/auth/send_again/');
            ...
            if ($this->input->post() && !empty($id) && $proceed) {
                ...
                $db_data_user['user_level'] = $this->input->post('user_level');
                ...
                if ($this->User_model->setRecordForUserProfiles($db_data_profile, $userid)) {
                    ...
                    if ($db_data_user['password'] && $db_data_user['old_password']) {
                        ...
                        //$update_user_active = $this->tank_auth->updateuseractive($db_data_user['user_level'], $db_data_user['activated']);
                    ...
                    // Update user status and user level ////ake 1.4.14
                    $this->User_model->setRecord(array('activated' => $db_data_user['activated']), $userid);
                    $this->User_model->setRecord(array('user_level' => $db_data_user['user_level']), $userid);
                    
                    
                    
//                    if ($db_data_user['user_level'] && $db_data_user['activated']) {
//                        if ($this->tank_auth->is_admin() && ($user_profile->id != $this->tank_auth->get_user_id()))
//                            {
//                            $this->User_model->setRecord(array('activated' => $db_data_user['activated']), $userid);
//                            $this->User_model->setRecord(array('user_level' => $db_data_user['user_level']), $userid);
//                            }
