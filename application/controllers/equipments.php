...
class Equipments extends CI_Controller {
    ...
    function __construct() {
        ...
        if ($this->tank_auth->is_admin() || $this->tank_auth->is_adminperalatan()){
        }
        else {
            redirect('auth/login');
        }
    ...
    function create($id = 0) {
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('no_ruj', 'No Rujukan', 'required');
	    ...
	    $this->form_validation->set_rules('description', 'Penerangan', 'trim');
            ...
            if ($this->form_validation->run() != FALSE) {
                ...
                $db_data['model'] = $this->input->post('model');
                $db_data['no_siri'] = $this->input->post('no_siri');
                $db_data['no_ruj'] = $this->input->post('no_ruj');
