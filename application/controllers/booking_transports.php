...
class Booking_transports extends CI_Controller {
        ...
        function status($status_id = 0, $page = '') {
                ...
                $data['outstation'] = $this->Booking_transport_model->OutStation;
                $data['transport_types'] = $this->Booking_transport_model->transport_types;
                //$data['driver'] = $this->Transport_type_model->getAllRecords();
                //$data['driver'] = $this->Booking_transport_model->get_driver_old();
        ...
        function create($id = 0) {
                if ($this->input->post()) {
                        ...
                        if ($this->form_validation->run() != FALSE) {
                                ...
                                $db_data_booking['driver'] = $this->input->post('driver');
                                ...
                                $db_data_booking_transports['outstation'] = $this->input->post('outstation');
                                //$db_data_booking_transports['driver'] = $this->input->post('driver');
...
//                                $db_data_booking_transports['transports'] = $transport_list;
                                ...
                                if ($this->Booking_transport_model->setRecord($db_data_booking_transports, $id)) {
                                        ...
                                        if ($this->Booking_model->setRecord($db_data_booking, $id)) {
                                                ...
                                                $head_driver = $this->User_model->getRecordByUserLevel('3');
                                                $admin_list = $this->User_model->getRecordByUserLevel('1');
                                                
                                                $to = $head_driver->email;
                                                $cc = $admin_list->email;
                                                
                                                $this->_send_email('booking-transport',$to,$cc, $content_data);
                                                /*                                                 * ******** Email Section ends ********* */
        ...
        function view($id) {
                ...
                $driverNo = $this->User_model->getEmelNotelSec($booking->driver);
                $data['driverNo'] = $driverNo; 
                //enable ni utk amik name driver from id. tp x stable lg if enable akan ade few error
                //$driverDetail  = $this->User_model->getRecordById3($booking->driver);
                ...
                if ($this->input->post()) {
                        ...
                        if ($this->form_validation->run() != FALSE) {
                                ...
                                if ($this->Booking_model->setBookingStatus($db_data_booking, $id)) {
                                        ...
                                        $content_data2['site_name'] = $site_name;
                                        $content_data2['booking_id'] = $id;
                                        $content_data2['booking_ref_no'] = $booking_ref_no;
                                        
                                        $this->_send_email2($booking_notification_type,$booker->email,$content_data2);
                ...
                $data['outstation'] = $this->Booking_transport_model->OutStation;
                //$data['driver'] = $this->Transport_type_model->getAllRecords();
                $data['driver'] = $this->Booking_transport_model->get_driver($booking->start_date , $booking->end_date);
        ...
        function _send_email($type, $email,$emailcc, &$data) {
            
        $list = array($emailcc,'braathaaake@gmail.com');
        
        $this->email->from($this->config->item('webmaster_email', 'tank_auth'), $this->config->item('website_name', 'tank_auth'));
        $this->email->reply_to($this->config->item('webmaster_email', 'tank_auth'), $this->config->item('website_name', 'tank_auth'));
        $this->email->to($email);
        $this->email->cc($list);
        //$this->email->bcc('braathaaake@gmail.com');
    ...
    function _send_email2($type, $email, &$data) {
            
        $list = 'braathaaake@gmail.com';
        ...
        //$this->email->bcc('braathaaake@gmail.com');
        $this->email->subject(sprintf($this->lang->line('auth_subject_' . $type), $this->config->item('website_name', 'tank_auth')));
        $this->email->message($this->load->view('email/' . $type . '-trans-html.php', $data, TRUE));
        //$this->email->set_alt_message($this->load->view('email/' . $type . '-txt', $data, TRUE));
        $this->email->send();
